...
import pandas as pd
import datetime
from functions import last_day_of_month, datareader
import numpy as np
...
all_files = datareader(path)

# Initial %: first all_files file readed
pesos_in = pd.read_csv(all_files[0], skiprows=2)
pesos_in = pesos_in.loc[:, ['Peso (%)', 'Ticker']]
pesos_in = pesos_in.set_index('Ticker')
pesos_in['Peso (%)'] = pesos_in['Peso (%)'] / 100

# All prices (2018-2020)
i = 0
...
df_pasiva['rendcum'] = np.round(final.values*100, 2)
